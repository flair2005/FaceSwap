...
Java_com_dongnao_facetrack_FaceHelper_loadModel(JNIEnv *env, jclass type, jstring detectModel_) {
...
JNIEXPORT void JNICALL
Java_com_dongnao_facetrack_FaceHelper_startTracking(JNIEnv *env, jclass type) {
...
JNIEXPORT void JNICALL
Java_com_dongnao_facetrack_FaceHelper_stopTracking(JNIEnv *env, jclass type) {
    if (tracker && isTracking) {
        isTracking = 0;
        tracker->stop();
    }
...
JNIEXPORT void JNICALL
Java_com_dongnao_facetrack_FaceHelper_detectorFace(JNIEnv *env, jclass type, jbyteArray data_,
...
JNIEXPORT void JNICALL
Java_com_dongnao_facetrack_FaceHelper_destory(JNIEnv *env, jclass type) {
    if (tracker)
        delete tracker;
    tracker = 0;
    if (nativeWindow)
...
JNIEXPORT void JNICALL
Java_com_dongnao_facetrack_FaceHelper_setSurface(JNIEnv *env, jclass type, jobject surface, jint w,
                                                 jint h) {
    if (surface && w && h) {
        if (nativeWindow) {
